...
BIGIP_GTM_MONITOR_HTTPS_ARGS = dict(
    cert=dict(type='str'),
...
class F5BigIpGtmMonitorHttps(F5BigIpNamedObject):
    def set_crud_methods(self):
        self.methods = {
            'create': self.mgmt_root.tm.gtm.monitor.https_s.https.create,
            'read': self.mgmt_root.tm.gtm.monitor.https_s.https.load,
            'update': self.mgmt_root.tm.gtm.monitor.https_s.https.update,
            'delete': self.mgmt_root.tm.gtm.monitor.https_s.https.delete,
            'exists': self.mgmt_root.tm.gtm.monitor.https_s.https.exists
        }


def main():
    module = AnsibleModuleF5BigIpNamedObject(argument_spec=BIGIP_GTM_MONITOR_HTTPS_ARGS, supports_check_mode=False)

    try:
        obj = F5BigIpGtmMonitorHttps(check_mode=module.supports_check_mode, **module.params)
        result = obj.flush()
