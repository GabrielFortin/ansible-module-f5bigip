...
class F5BigIpGtmMonitorHttp(F5BigIpNamedObject):
    def set_crud_methods(self):
        self.methods = {
            'create': self.mgmt_root.tm.gtm.monitor.https.http.create,
            'read': self.mgmt_root.tm.gtm.monitor.https.http.load,
            'update': self.mgmt_root.tm.gtm.monitor.https.http.update,
            'delete': self.mgmt_root.tm.gtm.monitor.https.http.delete,
            'exists': self.mgmt_root.tm.gtm.monitor.https.http.exists
        }


def main():
    module = AnsibleModuleF5BigIpNamedObject(argument_spec=BIGIP_GTM_MONITOR_HTTP_ARGS, supports_check_mode=False)
