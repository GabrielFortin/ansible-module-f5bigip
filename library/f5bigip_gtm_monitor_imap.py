...
DOCUMENTATION = '''
---
module: f5bigip_gtm_monitor_imap
short_description: BIG-IP gtm monitor imap module
description:
...
- name: Create GTM Monitor IMAP
  f5bigip_gtm_monitor_imap:
    ...
    f5_password: admin
    f5_port: 443
    name: my_imap_monitor
    partition: Common
    description: My imap monitor
    state: present
...
class F5BigIpGtmMonitorImap(F5BigIpNamedObject):
    def _set_crud_methods(self):
        self._methods = {
            'create': self._api.tm.gtm.monitor.imaps.imap.create,
            'read': self._api.tm.gtm.monitor.imaps.imap.load,
            'update': self._api.tm.gtm.monitor.imaps.imap.update,
            'delete': self._api.tm.gtm.monitor.imaps.imap.delete,
            'exists': self._api.tm.gtm.monitor.imaps.imap.exists
        }
...
def main():
    ...
    try:
        obj = F5BigIpGtmMonitorImap(check_mode=module.check_mode, **module.params)
        result = obj.flush()
