...
BIGIP_GTM_MONITOR_POSTGRESQL_ARGS = dict(
    count=dict(type='int'),
...
class F5BigIpGtmMonitorPostgresql(F5BigIpNamedObject):
    def set_crud_methods(self):
        self.methods = {
            'create': self.mgmt_root.tm.gtm.monitor.postgresqls.postgresql.create,
            'read': self.mgmt_root.tm.gtm.monitor.postgresqls.postgresql.load,
            'update': self.mgmt_root.tm.gtm.monitor.postgresqls.postgresql.update,
            'delete': self.mgmt_root.tm.gtm.monitor.postgresqls.postgresql.delete,
            'exists': self.mgmt_root.tm.gtm.monitor.postgresqls.postgresql.exists
        }


def main():
    module = AnsibleModuleF5BigIpNamedObject(argument_spec=BIGIP_GTM_MONITOR_POSTGRESQL_ARGS, supports_check_mode=False)
