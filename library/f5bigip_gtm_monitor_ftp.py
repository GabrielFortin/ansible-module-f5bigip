...
class F5BigIpGtmMonitorFtp(F5BigIpNamedObject):
    def set_crud_methods(self):
        self.methods = {
            'create': self.mgmt_root.tm.gtm.monitor.ftps.ftp.create,
            'read': self.mgmt_root.tm.gtm.monitor.ftps.ftp.load,
            'update': self.mgmt_root.tm.gtm.monitor.ftps.ftp.update,
            'delete': self.mgmt_root.tm.gtm.monitor.ftps.ftp.delete,
            'exists': self.mgmt_root.tm.gtm.monitor.ftps.ftp.exists
        }


def main():
    module = AnsibleModuleF5BigIpNamedObject(argument_spec=BIGIP_GTM_MONITOR_FTP_ARGS, supports_check_mode=False)
