...
DOCUMENTATION = '''
---
module: f5bigip_gtm_monitor_oracle
short_description: BIG-IP gtm monitor oracle module
description:
...
class F5BigIpGtmMonitorOracle(F5BigIpNamedObject):
    def _set_crud_methods(self):
        self._methods = {
            'create': self._api.tm.gtm.monitor.oracles.oracle.create,
            'read': self._api.tm.gtm.monitor.oracles.oracle.load,
            'update': self._api.tm.gtm.monitor.oracles.oracle.update,
            'delete': self._api.tm.gtm.monitor.oracles.oracle.delete,
            'exists': self._api.tm.gtm.monitor.oracles.oracle.exists
        }
...
def main():
    ...
    try:
        obj = F5BigIpGtmMonitorOracle(check_mode=module.check_mode, **module.params)
        result = obj.flush()
